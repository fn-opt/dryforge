...
## Delivery — use structured choices when available

When a question has clear, enumerable options (stack choice, persistence approach, scope
boundaries, yes/no trade-offs), prefer your platform's structured user-input tool for choices when available; otherwise ask a concise plain-text question. The goal is to keep the decision crisp
instead of hiding the actual fork in long prose.
...
- For questions where multiple answers are valid and the UI supports it, allow multiple selections.
