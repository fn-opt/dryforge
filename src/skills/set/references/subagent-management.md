# subagent-management.md — reading the project with bounded subagents

The project-reading phase (set's **GATHER** / ready's **EXPLORE**) protects
main context by using **read-only** summaries when broad project reading would otherwise flood the
main session. Subagents are an optimization, not a ceremony: use them when summary isolation or
parallel reading pays for its dispatch cost. Raw code should not enter main unless a later narrow
pass genuinely needs it.

## GATHER budget

Before dispatching, do a cheap inline map: repo instructions, file list, manifests, verification
scripts, and source paths named by the spec/plan. Delegate only when the map shows that the read is
broad, independent, and answerable as a compact summary. If the target is small, obvious, or already
named by the input documents, read it inline.
...
|---|---|---|
| **Scout** (first) | project layout, conventions, entry points. Use the project agent guide (`AGENTS.md` or `CLAUDE.md`) if present; otherwise explore broadly and tell the user you are reading the whole project. | structure + rules/conventions + locations of modules relevant to the spec/plan |
...
Every dispatch must pass a **Dispatch ROI** check: the prompt names the exact question, the scope is
bounded, the returned summary will change verification/refinement/dependency decisions, and the
expected summary is smaller than the raw context it replaces.
